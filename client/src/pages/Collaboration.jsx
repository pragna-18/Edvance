...
import { FaSave, FaUsers, FaComment, FaArrowLeft, FaTimes, FaCheck } from 'react-icons/fa';
...
const Collaboration = () => {
  ...
  const fetchComments = async () => {
    try {
      const response = await axios.get(`${API_URL}/collaboration/${id}/comments?resolved=false`);
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await axios.put(`${API_URL}/lesson-plans/${id}`, {
        title: plan.title,
  ...
  const handleAddComment = async () => {
    ...
    try {
      const response = await axios.post(`${API_URL}/collaboration/${id}/comments`, {
        content: newComment,
  ...
  const handleResolveComment = async (commentId) => {
    try {
      await axios.put(`${API_URL}/collaboration/${id}/comments/${commentId}`, {
        resolved: true
      });
